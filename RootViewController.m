...
//Updated in 2018 & 2019 by @TD_Advocate
//Huge thanks to @TheTomMetzger, @Skittyblock, & @SparkDev for their massive amounts of help with updating and adding new features
//Shoutout to @xerusdesign for making the new icons for the updates
...
- (void)viewDidLoad {
    ...
    SparkAppListTableViewController* apps = [[SparkAppListTableViewController alloc] init];
    
    /*
    SparkAppList* apps = [[SparkAppList alloc] init];

    [apps getAppList: ^(NSArray* arr){ 
    }];
    */
    //Creating Dictionary to pull app list
    theApps = [[NSMutableDictionary alloc] init];
    for(int i = 0; i < apps.appList; i++) {
        NSString* name = [[apps.appList allValues] objectAtIndex:i];
        if([excludedApps containsObject:name]) {
            continue;
        }
        theApps[[[apps.displayName allKeys] objectAtIndex:i]] = name;
    }
